...
const CONTENT_FILE_PATH_QUERY = `?__contentFilePath=`; // Split the path component URI without using the expensive URI.parse()

export const splitComponentPath = componentPath => {
  ...
  const splitPath = cleanedComponentPath.split(CONTENT_FILE_PATH_QUERY); // We only support URI paths with the `?__contentFilePath=` parameter
...
export const getPathToLayoutComponent = componentPath => splitComponentPath(componentPath)[0]; // Get the path to the content file, falling back to the js component if no content file is given.
// Pages directly created from `.mdx` files

export const getPathToContentComponent = componentPath => {
