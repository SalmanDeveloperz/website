...
exports.pluginOptionsSchema = function (_ref) {
  ...
  return Joi.object({
    ...
    sizeByPixelDensity: Joi.boolean().default(false).description("[deprecated] Pixel density is only used in vector images, which Gatsby\u2019s implementation of Sharp doesn\u2019t support. This option is currently a no-op and will be removed in the next major version of Gatsby."),
    ...
    tracedSVG: Joi.alternatives().try(Joi.boolean(), Joi.object({
      ...
      "black", "white", "left", "right", "minority", "majority").default("majority"),
      turdSize: Joi.number().default(100),
      alphaMax: Joi.number(),
      optCurve: Joi.boolean().default(true),
      optTolerance: Joi.number().default(0.4),
      threshold: Joi.alternatives().try(Joi.number().min(0).max(255), Joi.number().valid(-1)).default(-1),
      ...
      if (!!value && !process.env.GATSBY_WORKER_ID) {
        console.warn("\"tracedSVG\" plugin option for \"gatsby-remark-images\" is no longer supported. Blurred placeholder will be used. See https://gatsby.dev/tracesvg-removal/");
      }

      return undefined;
    }).description("Use traced SVGs for placeholder images instead of the \u201Cblur up\u201D effect. Pass true for traced SVGs with the default settings (seen here), or an object of options to override the default. For example, pass { color: \"#F00\", turnPolicy: \"TURNPOLICY_MAJORITY\" } to change the color of the trace to red and the turn policy to TURNPOLICY_MAJORITY. See node-potrace parameter documentation for a full listing and explanation of the available options."),
